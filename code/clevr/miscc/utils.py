...
def load_validation_data(filenames, index):
    key = filenames[index]
    with open(key, "rb") as f:
        json_file = json.load(f)
    img_name = self.img_dir + "/" + json_file["image_filename"]
    img, flip_img = get_img(img_name)
    ...
    return img, transformation_matrices, label
...
def compute_discriminator_loss(netD, real_imgs, fake_imgs,
    ...
    # real_logits = torch.clamp(real_logits, 1e-8, 1-1e-8)
