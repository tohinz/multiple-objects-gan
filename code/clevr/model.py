    ...
    def __init__(self, ndf, nef, bcondition=True):
        ...
        if bcondition:
            self.outlogits = nn.Sequential(
                conv3x3(ndf * 8 + 13, ndf * 8),
                nn.BatchNorm2d(ndf * 8),
                nn.LeakyReLU(0.2, inplace=True),
                nn.Conv2d(ndf * 8, 1, kernel_size=4, stride=4))
        else:
            self.outlogits = nn.Sequential(
                nn.Conv2d(109, 1, kernel_size=4, stride=4))

    def forward(self, h_code, c_code=None):
        # conditioning output
        if self.bcondition and c_code is not None:
            c_code = c_code.view(h_code.shape[0], 13, 1, 1)
...
class BBOX_NET(nn.Module):
    ...
    def forward(self, labels, transf_matr_inv, num_objects):
...
class STAGE1_G(nn.Module):
    ...
    def forward(self, noise, transf_matrices_inv, label_one_hot, num_objects=4):
        local_labels = torch.cuda.FloatTensor(noise.shape[0], num_objects, self.ef_dim).fill_(0)

        # local pathway
        h_code_locals = torch.cuda.FloatTensor(noise.shape[0], self.gf_dim // 4, 16, 16).fill_(0)
        for idx in range(num_objects):
            current_label = self.label(label_one_hot[:, idx])
        ...
        if cfg.USE_BBOX_LAYOUT:
            bbox_code = self.bbox_net(local_labels, transf_matrices_inv, num_objects)
...
class STAGE1_D(nn.Module):
    ...
    def define_module(self):
        ...
        self.local = nn.Sequential(
            nn.Conv2d(3 + linput, ndf * 2, 4, 1, 1, bias=False),
    ...
    def _encode_img(self, image, label, transf_matrices, transf_matrices_inv, num_objects=4):
        # local pathway
        h_code_locals = torch.cuda.FloatTensor(image.shape[0], self.df_dim * 2, 16, 16).fill_(0)

        for idx in range(num_objects):
            current_label = label[:, idx].view(label.shape[0], 13, 1, 1)
            current_label = current_label.repeat(1, 1, 16, 16)
